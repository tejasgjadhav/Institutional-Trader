...
| Stock credit **v0** (c/w 0.35–0.40) ⚠️ EXPERIMENTAL | `STOCK_CREDIT_V0_ENABLED` | 77% IS / 91% OOS | ~₹1,800 | **WEAK** · IS +1.9% ROM, +ve only 4/6 yrs (n=310); OOS n=43. Adds ~9% net for ~99% capital — one more CORE lot returned 82% more. User-approved 2026-07-31 to build a live record. See LOWCW_BAND_RESCUE §7 |
...
**≈ ₹37,924/mo model at 1 lot** (v0 would add ~₹1,800 but roughly doubles stock-credit margin — it is a
track-record experiment, not a profit plan; scale CORE lots before funding it). Edges are validated; **magnitude is NOT** — v2 implies ~97%/mo on
