"""Oct'24->date era of the D5/D10/D15/D20 standalone-Donchian study (real Upstox expired-instrument
premiums). IDENTICAL engine to stkfade_d5_vs_d10.py — only CONFIGS extended to all four windows so
D5/D10 reproduce the known cross-check (D5=203/87.2%/+31.5%w, D10=148/88.5%/+31.5%w). Each window is
STANDALONE (its own breakout stream + own re-entry gap), NOT the union. Deployed v2 config: short
2-OTM, width 4, TP 50% of credit, stop 3x, gate credit/width>=0.40 + short prem>=50, min-DTE 10,
reentry 3d. Costs 2.5% slippage + Rs20x4/lot. Settle intrinsic at expiry. Out: /tmp/d5_10_15_20_oct24.json
"""
...
CONFIGS = [("D5", 5), ("D10", 10), ("D15", 15), ("D20", 20)]
...
def do_stock(sym0):
    ...
    with LK:
        DONE[0] += 1
        if DONE[0] % 20 == 0:
            json.dump(out, open("/tmp/d5_10_15_20_oct24.json", "w"))
            print(f"  ...{DONE[0]}/100 | " + " ".join(f"{c}={len(out[c])}" for c,_ in CONFIGS), flush=True)

with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ex:
    list(ex.map(do_stock, UNIVERSE))
json.dump(out, open("/tmp/d5_10_15_20_oct24.json", "w"))

print(f"\n=== v2 fade (gate>=0.40) STANDALONE D5/D10/D15/D20, Oct'24->date ===")
