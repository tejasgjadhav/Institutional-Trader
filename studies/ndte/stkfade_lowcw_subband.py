"""LOW-C/W SUB-BAND SPLIT — OOS (Upstox expired options, Oct'24 -> date).

stkfade_lowcw_oos2.py pooled 0.30-0.40 and the rescue failed. CW_BUCKET_ANALYSIS reports the two
halves behave very differently (0.35-0.40: 77% win/+9.2%w · 0.30-0.35: 78%/+1.1%w), so a live
upper half could have been buried under a dead lower half. This splits them.

Reuses /tmp/lowcw_legcache.pkl, so it costs almost no new API calls.
...
CONFIGS = [
    ("UPPER 0.35-0.40  S2/W4 TP-50 stop-3x [do nothing]", "b35", 2, 4, 0.50, 3.0),
    ("UPPER 0.35-0.40  S2/W4 TP-40 no-stop",              "b35", 2, 4, 0.40, None),
    ("UPPER 0.35-0.40  S2/W1 TP-40 no-stop",              "b35", 2, 1, 0.40, None),
    ("UPPER 0.35-0.40  S2/W1 TP-50 no-stop",              "b35", 2, 1, 0.50, None),
    ("LOWER 0.30-0.35  S2/W4 TP-50 stop-3x [do nothing]", "b30", 2, 4, 0.50, 3.0),
    ("LOWER 0.30-0.35  S2/W4 TP-40 no-stop",              "b30", 2, 4, 0.40, None),
    ("LOWER 0.30-0.35  S2/W1 TP-40 no-stop",              "b30", 2, 1, 0.40, None),
    ("LOWER 0.30-0.35  S2/W1 TP-50 no-stop",              "b30", 2, 1, 0.50, None),
]
...
BYSYM = "/tmp/lowcw_subband_bysym.json"
...
def do_stock(sym0):
    ...
    if sym in DONE_SYM:
        with LK: DONE[0] += 1
        return
    mine = collections.defaultdict(list); myseen = {"gate": 0, "b35": 0, "b30": 0, "low": 0}
    try:
    ...
    for i in range(20, len(u) - 1):
        ...
        last[typ] = day
        cw = rcredit / rw
        pop = "gate" if cw >= 0.40 else ("b35" if cw >= 0.35 else ("b30" if cw >= 0.30 else "low"))
        myseen[pop] += 1
        if pop in ("low", "gate"): continue
    ...
    with LK:
        ...
        DONE[0] += 1
        if DONE[0] % 3 == 0:
            print(f"  …{DONE[0]}/{len(SUBSET)} stocks · upper={seen['b35']} lower={seen['b30']} "
                  f"low={seen['low']} · legs cached {len(LEGC)}", flush=True)
            json.dump({str(k): v for k, v in out.items()}, open("/tmp/lowcw_subband.json", "w"))
            _save_cache()
...
print(f"\n=== OOS Oct'24 -> Jul'26 · signals: gate={seen['gate']}  "
      f"UPPER 0.35-0.40={seen['b35']}  LOWER 0.30-0.35={seen['b30']}  below={seen['low']} ===")
