"""OOS confirmation for the NIFTY fade IS-grid survivor (see niftyfade_grid.py):
DC5 breakout, short 2-OTM width 4, nearest expiry >= 20 DTE, c/w gate sweep, four exits.
Same n-preserving harness as idxfade_oos_exits.py: fetch each trade's daily premium path
ONCE from Upstox expired-instruments (Oct'24->date), then evaluate gate x exit cells on
the SAME trades. Entry at close, reentry 3d/side, intrinsic settle, spf slippage."""
...
SYM = "NIFTY"
START = date(2024, 10, 1); MIN_DTE = 20; REENTRY = 3
DC, SHORT, WIDTH = 5, 2, 4
GATES = [0.0, 0.40, 0.45, 0.50]
EXITS = [("hold/stop2", None, 2.0), ("TP75/stop2", 0.75, 2.0),
         ("hold/stop3", None, 3.0), ("hold/nostop", None, None)]
OUT = "/private/tmp/claude-501/-Users-sayali-files/d62dc509-cfa1-4925-8c10-c57a2151b1c9/scratchpad/niftyfade_oos_dc5s2w4d20.json"
...
if os.path.exists(OUT):
    ...
    print(f"loaded cached {len(recs)} trades from {OUT}", flush=True)
else:
    print(f"NIFTY fade OOS fetch Oct'24->date (DC{DC} s{SHORT} w{WIDTH}, >= {MIN_DTE} DTE)…", flush=True)
    ...
    if not exps_all:
        print("FATAL: no expiries from expired-instruments API", flush=True); sys.exit(1)
    ...
    for i in range(max(DC, 20), len(u) - 1):
        ...
        to = min(datetime.fromisoformat(exp).date(), day + timedelta(days=60)).isoformat()
        ...
        print(f"  {day} {typ} exp {exp} short {sk:.0f} credit {credit:.1f} w {w:.0f} c/w {credit/w:.2f}", flush=True)
        json.dump(recs, open(OUT, "w"))
    json.dump(recs, open(OUT, "w"))

print(f"\n{len(recs)} trades fetched", flush=True)
...
if cws:
    print(f"c/w distribution (n={len(cws)}): min {cws[0]:.2f} p25 {cws[len(cws)//4]:.2f} "
          f"median {cws[len(cws)//2]:.2f} p75 {cws[3*len(cws)//4]:.2f} max {cws[-1]:.2f}", flush=True)

print(f"\n=== NIFTY fade OOS Oct'24->date, DC{DC} s{SHORT}w{WIDTH} DTE>={MIN_DTE} — gate x exit ===")
for g in GATES:
    sub = [r for r in recs if r["cw"] >= g]
    for lab, tp, stop in EXITS:
        ...
        n = len(nets)
        if not n:
            print(f"cw>={g:.2f} {lab:12s} n=0", flush=True); continue
        yr = "  ".join(f"{y}:{sum(a for a,_ in gg)/sum(b for _,b in gg)*100:+.1f}%w(n={len(gg)})"
                       for y, gg in sorted(ys.items()))
        print(f"cw>={g:.2f} {lab:12s} n={n:3d} win {wins/n*100:5.1f}%  net {sum(a for a,_ in nets)/wsum*100:+6.1f}%w   [{yr}]", flush=True)
print("DONE-NIFTYFADE-OOS", flush=True)
