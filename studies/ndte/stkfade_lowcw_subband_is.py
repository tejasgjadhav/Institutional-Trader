"""LOW-C/W SUB-BAND SPLIT — in-sample twin of stkfade_lowcw_subband.py.

The 432-cell sweep pooled 0.30-0.40. CW_BUCKET_ANALYSIS says the halves differ a lot, so this
splits them: does the UPPER half (0.35-0.40) carry the in-sample result, and does it survive?
...
for (day, sym, typ, c) in sigs:
    ...
    cw = credit / w
    pops["all"].append(rec)
    pops["gate" if cw >= 0.40 else ("b35" if cw >= 0.35 else ("b30" if cw >= 0.30 else "low"))].append(rec)
...
print("\n" + "=" * 78)
print("IN-SAMPLE 2019->Sep'24 — the band SPLIT into halves")
print("=" * 78)
for tag, lab in (("b35", "UPPER 0.35-0.40"), ("b30", "LOWER 0.30-0.35")):
    print(f"\n--- {lab}  (n={len(pops[tag])} signals) ---")
    rep(evaluate(pops[tag], 2, 4, 0.50, 3.0), f"  {lab} @ S2/W4 TP-50 stop-3x  [do nothing]")
    rep(evaluate(pops[tag], 2, 4, 0.40, None), f"  {lab} @ S2/W4 TP-40 no-stop")
    rep(evaluate(pops[tag], 2, 1, 0.40, None), f"  {lab} @ S2/W1 TP-40 no-stop")
    rep(evaluate(pops[tag], 2, 1, 0.50, None), f"  {lab} @ S2/W1 TP-50 no-stop")
print("\nDONE-SUBBAND-IS")
