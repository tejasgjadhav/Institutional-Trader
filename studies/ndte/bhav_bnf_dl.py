...
(holiday-shifted expiries). Resumable -> /tmp/ndte_bhav_bnf/{date}.csv"""
import os, io, zipfile, time
import requests, pandas as pd
from datetime import date, timedelta

CACHE = "/tmp/ndte_bhav_bnf"; os.makedirs(CACHE, exist_ok=True)
...
def one(d):
    ...
    for attempt in range(3):
        try:
            ...
            sub = df[(df["SYMBOL"] == "BANKNIFTY") & (df["INSTRUMENT"] == "OPTIDX") &
...
n = ok = 0
while d <= end:
    if d.weekday() in (2, 3):                      # Thursday
        r = one(d); n += 1
        got = r.startswith("ok") or r == "cached"
        if not got:                           # holiday-shifted expiry -> Wednesday
            r2 = one(d - timedelta(days=1)) if d.weekday()==3 else "skip"
