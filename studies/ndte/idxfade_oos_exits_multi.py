"""OOS exit sweep for the INDEX swing fade — parameterized by symbol (Part 2: BANKNIFTY, SENSEX).
Copy of idxfade_oos_exits.py (the NIFTY run) with SYM from argv. Same n-preserving harness:
fetch each trade's daily premium path ONCE from Upstox expired-instruments (Oct'24->date), then
evaluate ALL candidate exits on the SAME trades. Geometry mirrors the deployed
...
NO credit/width or premium gate; each trade's c/w is recorded.
Notes vs NIFTY: BANKNIFTY (NSE) is monthly-only expiry since late'24 (100-pt strikes);
SENSEX (BSE_FO) is weekly-Thu, options only exist since ~2023 (window here starts Oct'24 anyway).
Usage: python idxfade_oos_exits_multi.py BANKNIFTY|SENSEX"""
...
OUT = f"/private/tmp/claude-501/-Users-sayali-files/d62dc509-cfa1-4925-8c10-c57a2151b1c9/scratchpad/idxfade_oos_trades_{SYM}.json"
...
print(f"DONE-IDX-OOS-EXITS-{SYM}")
